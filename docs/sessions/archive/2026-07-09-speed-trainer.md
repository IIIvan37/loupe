...
- **Tempo floor lowered 50 % → 40 %** (`MIN_PLAYBACK_RATE` 0.4): fine
  ...
  hardcoding 50/150. First shipped at 25 %, **raised to 40 % on the user's
  ear check the same day** (SoundTouch artefacts too heavy at quarter speed).
...
- **Same-session user feedback, three fixes** (test-first, on the PR):
  1. Floor 25 % → **40 %** (above).
  2. **« Arrêter la rampe » restores the tempo memorised at arming** — the
     ramp borrows the tempo, it never keeps it. Every stop seam restores
     (toggle off, passage replaced, cleared); a slider takeover still wins
     (stop restores first, the user's choice applies right after); an idle
     stop applies nothing.
  3. **An enabled loop confines the playhead**: `wrapToLoop` now pulls a
     position BEFORE the loop up to its start (fresh arm ahead of the cursor,
     click outside), like the wrap at the end; a pull-up earns no practice
     pass (`completesLoopPass` gate untouched).

## Not done / remaining
- **Browser pass on the Mac** (skipped here per the
  browser-verify-only-hard-cases rule — jsdom already drives the whole flow):
  the roadmap's « vérifier la qualité » question is **answered** (25 % too
  degraded → 40 % validated by ear). Remaining eyes-only items: the ramp
  popover + running read-out in the tight loop-controls row (reuses the
  rename-popover skins, low risk), and optionally VoiceOver on the ramp
  announcements.
...
- **The ramp stops when its premise dies** — one rule, five seams: loupe
  ...
  takeover, project open. **Stopping restores the tempo memorised at
  arming** (user decision — first cut kept the earned tempo); a slider
  takeover wins over the restore by ordering.
...
- **An enabled loop confines the playhead** (user decision): outside
  `[start, end)` → repositioned at the loop start, both sides. Escaping is
  what the loop toggle is for.
...
## Gate status
- typecheck: ✅
- tests (with coverage): ✅ **712 passed** (74 files; +34 this session) —
  coverage 96,04 % stmts / 88,94 % branches (thresholds 85/80)
- mutation (Stryker, local — core touched): ✅ **95,20 %** overall;
  `speed-trainer.ts` **100 %** (39 mutants); `playback-rate.ts` 87,5 % (2
  equivalent boundary survivors, hand-verified); `loop-region.ts` 95 % (1
  pre-existing equivalent survivor on `makeLoopRegion`'s edge ordering)
- biome / sheriff / knip / jscpd: ✅ / ✅ / ✅ / ✅ 5 clones (unchanged);
  react-doctor ✅ 0 (after moving the ramp apply back into the handlers)

## State to resume from
- **Single next action**: merge **PR #74** (opened, CI green; three
  user-feedback commits landed after opening — floor 40 %, stop-restores
  -tempo, loop confinement). After merge: **Lot I.2** (tempo manuel — tap-tempo, saisie
...
- Gotchas: `useSpeedTrainer`'s `stateRef` is the source of truth and
  ...
  returns. The floor is **40 %, validated by ear** (25 % rejected). `wrapToLoop`
  now CONFINES (both sides) — any future consumer expecting pass-through
  below the loop start must not use it.
