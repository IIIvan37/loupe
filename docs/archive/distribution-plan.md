...
| D6 | Validation plateformes — outillage (#282 : build `workflow_dispatch`, job CI `Rust Windows`, checklist, audit) ; 3 bugs levés en VM (#283 zip Windows, #284 zip-dans-zip, #285 auth OTP + `127.0.0.1`) ; **parcours complet OK sur Windows 11 ARM (2026-07-28)**. Ubuntu ARM64 non rejoué (test qemu faible), cible `aarch64-linux` native différée | ✅ #282–#285 |
| Release | **v0.1.0 publiée (2026-07-31)** : tag → 3 binaires + `SHA256SUMS` + `loupe.rb` en Release ; tap `iiivan37/loupe` en ligne, `brew install loupe` vérifié (`brew trust` requis, noté RELEASING). Étape tap du workflow à réparer pour la prochaine (PAT sans accès au dépôt tap — formule poussée à la main cette fois) | ✅ tag v0.1.0 |
