...
| `sessionSaveInput` | `(session: LiveSessionSnapshot) => Omit<SaveProjectInput, 'stamp'>` | Politiques-au-core lot (from web `project-session.ts`) — turn the live session into `saveProject`'s input: source bytes plus, when stems are mixing, each mixer-held stem re-encoded as WAV bytes (`encodeWav`) — exactly the pair `mixerMatchesStems` checks. Pure mapping, no ports. |
| `restoreSession` | `(opened, deps: SessionRestoreDeps) => Promise<void>` | Politiques-au-core lot (from web `project-session.ts`) — rebuild the working session from an `openProject` result, behind driven ports in core vocabulary (`importAudio`, `markers`/`loops`/`mixer`/`separation` seats, `tempo.set`/`detect`, `metronome.enable`/`attach`, the auto-detect guard, the awaited `onRestoreStep` narration): re-import the bytes (a superseded import restores nothing and re-arms auto-detect), seat markers/loops/tuning/chart, relink the persisted loupe to the library loop it float-equals, decode each stored stem WAV back to PCM (narrated), replay the separation, then the tempo fast path (persisted → `tempo.set` + sanitized grid + metronome seat; old manifest → fire-and-forget `detect`, `metronome` handed `undefined` so the ADAPTER seats its default-muted click — the lane id is the mixer adapter's, ADR 0012). The web `project-session.ts` keeps only the `File` wrap, the paint pacing, and the i18n'd structure-kind re-adoption. Related pure domain: `sessionSignature` (project/domain) — the canonical fingerprint of what a save persists (« Enregistré » read-out), signing the SAME manifest defaults the restore seats (`tuningOrDefault`, `fineTuneOrDefault`, `sanitizeBeatGrid`, `DEFAULT_BEATS_PER_BAR`, `DEFAULT_METRONOME_SETTINGS`) — one definition of the canonical persisted state. |
...
> (`TransportState` machine), the playback-parameter clamps `clampPlaybackRate`
> (tempo ratio) and the speed-trainer policies — Slice 3 + revue-justesse P2.
> `formatTimecode` (m:ss) and `clampPitchSemitones` stay in the nursery
> (`timecode` still awaits its second consumer). Politiques-au-core lot: the playhead streams at frame
> rate OUTSIDE the reducer (Lot L.1), so what one streamed frame MEANS — wrap
> the armed loop (earning a speed-trainer pass or not), stop at the end of a
> real timeline — is `resolvePlaybackTick` (playback-tick.ts); the web's
> position listener only executes its outcome, and the reducer's former dead
> `tick` action is gone.
>
...
> listener (`onLoopWrap`) and lands every earned step on the player. When the
> ramp stops is the core's single rule (politiques-au-core lot):
> `speedTrainerSurvives(seam: SpeedTrainerSeam)` — every adapter site that
> mutates the loupe, toggles looping or takes the tempo back names its seam
> (`cross(seam)`) and the rule decides; only an edge adjustment of the same
> passage and re-arming looping keep the practice alive.
>
...
| `TrackSource` | driven | `web`: `createHttpTrackSource` against the local server (J4.1 — `{url}` POST `/download` → streamed NDJSON progress → `done` line with a content-addressed `ref` the adapter `GET`s from `/audio/{ref}`, yielding encoded m4a/AAC bytes + metadata). The server runs **yt-dlp**. Desktop twin `createTauriTrackSource` (T2.3 — a Rust `download_track` command drives a managed, self-updating yt-dlp binary under app-data at the server's guard parity, streams progress over a Tauri `Channel`, and the finished file is read back through the fs plugin then its temp dir removed). A cloud API could be a later adapter on the same port. |
