...
describe('resolvePlaybackTick', () => {
  it('advances inside the timeline', () => {
    expect(resolvePlaybackTick(tick({ atSeconds: 6 }))).toEqual({
      kind: 'advance',
      endReached: false
    })
  })
  ...
  it('reaching the end of a real timeline while playing stops playback', () => {
    expect(resolvePlaybackTick(tick({ atSeconds: 10 }))).toEqual({
      kind: 'advance',
      endReached: true
    })
  })
  ...
  it('never reads the end on an empty (zero-duration) timeline', () => {
    expect(
      resolvePlaybackTick(tick({ atSeconds: 0, durationSeconds: 0 }))
    ).toEqual({ kind: 'advance', endReached: false })
  })

  it('does not re-stop a transport already paused at the end', () => {
    expect(
      resolvePlaybackTick(tick({ atSeconds: 10, isPlaying: false }))
    ).toEqual({ kind: 'advance', endReached: false })
  })
  ...
  it('wraps an enabled loop back to its start at the loop end', () => {
    expect(
      resolvePlaybackTick(tick({ atSeconds: 6.1, loop, loopEnabled: true }))
  ...
  it('wraps a seek landing far past the end without earning a pass', () => {
    ...
    expect(
      resolvePlaybackTick(tick({ atSeconds: 9, loop, loopEnabled: true }))
  ...
  it('pulls a position left before the loop up to its start, earning nothing', () => {
    expect(
      resolvePlaybackTick(tick({ atSeconds: 1, loop, loopEnabled: true }))
    ).toEqual({ kind: 'wrap', toSeconds: 2, completesPass: false })
  })

  it('plays straight through a disabled loop', () => {
    expect(
      resolvePlaybackTick(tick({ atSeconds: 7, loop, loopEnabled: false }))
    ).toEqual({ kind: 'advance', endReached: false })
  })
  ...
  it('ignores a degenerate zero-length loop (would wrap-seek every frame)', () => {
    ...
    expect(
      resolvePlaybackTick(
        tick({ atSeconds: 5, loop: degenerate, loopEnabled: true })
      )
    ).toEqual({ kind: 'advance', endReached: false })
  })
  ...
  it('advances inside an enabled loop', () => {
    expect(
      resolvePlaybackTick(tick({ atSeconds: 4, loop, loopEnabled: true }))
    ).toEqual({ kind: 'advance', endReached: false })
  })
