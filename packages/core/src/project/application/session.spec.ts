...
describe('restoreSession', () => {
  ...
  function fakeDeps(
    restored: RestoredSeparation | undefined,
    detected: TempoAnalysis | undefined = undefined
  ) {
    return {
      importAudio: vi.fn(async (_bytes: ArrayBuffer, _name: string) => audio),
      markers: { restore: vi.fn() },
      loops: { restore: vi.fn() },
      restoreActiveLoop: vi.fn(),
      restoreTuning: vi.fn(),
      restoreChordChart: vi.fn(),
      tempo: {
        detect: vi.fn(async (_audio: DecodedAudio) => detected),
        set: vi.fn()
      },
      metronome: { enable: vi.fn(), attach: vi.fn() },
      setSuppressAutoDetect: vi.fn(),
      separation: {
        restore: vi.fn(
          async (_audio: DecodedAudio, _sources: readonly SeparatedStem[]) =>
            restored
        )
      },
      mixer: { restore: vi.fn() }
  ...
  it('re-imports the bytes, restores markers/loops and seats the saved mixer', async () => {
    ...
    const restored: RestoredSeparation = { stems, sources: [voix] }
    ...
    expect(deps.importAudio).toHaveBeenCalledWith(
      opened.sourceBytes,
      'Mon projet'
    )
  ...
  it('narrates each stored stem before its synchronous decode (AS.4)', async () => {
    ...
    // the restore reports « piste n/total » ahead of each decode — and AWAITS
    // the adapter, so it can let the chip paint before the thread blocks.
  ...
  it('restores nothing when the re-import was superseded by a newer one', async () => {
    ...
    // supersede guard makes importAudio resolve undefined — the old project's
    // loops/markers must not land on the track imported meanwhile.
    const deps = {
      ...fakeDeps(undefined),
      importAudio: vi.fn(async () => undefined)
    }
    const opened: Extract<OpenProjectResult, { ok: true }> = {
      ok: true,
      project: baseProject,
    ...
    // The guard is disarmed so the newer track still auto-detects.
    expect(deps.setSuppressAutoDetect).toHaveBeenLastCalledWith(false)
  })
  ...
  it('detects the tempo for an old manifest and hands the adapter no channel', async () => {
    ...
    // Detection is fire-and-forget (it must not block the open). No channel is
    // forwarded: the adapter seats its own default-muted click (its id is the
    // mixer adapter's business).
    await vi.waitFor(() => {
      expect(deps.metronome.enable).toHaveBeenCalledWith(
        detected.grid,
        audio,
  ...
  it('seats nothing when the stored stems could not be rebuilt', async () => {
    // A corrupt stored WAV (or a superseded restore) resolves undefined: no
    // tempo may seat over an empty mix.
    const deps = fakeDeps(undefined)
    const opened: Extract<OpenProjectResult, { ok: true }> = {
      ok: true,
      project: { ...project, tempo: savedTempo },
    ...
    expect(deps.tempo.set).not.toHaveBeenCalled()
    expect(deps.metronome.attach).not.toHaveBeenCalled()
  })
})
