...
import { BarsPerRowField } from '../bars-per-row-field/bars-per-row-field.tsx'
import {
  DEFAULT_BARS_PER_ROW,
  readStoredBarsPerRow
} from '../bars-per-row-preference.ts'
import { chartHasContent } from '../chart-content.ts'
import { FormatHelpDialog } from '../format-help-dialog/format-help-dialog.tsx'
import { LeadSheet } from '../lead-sheet/lead-sheet.tsx'
import {
...
function KeyRow({
  ...
  /** The roman-numeral mode (AN.5): pressed state, or undefined while no
      {key} names the grid — no degree without a tonic, so the toggle waits. */
  readonly roman: boolean | undefined
  readonly onToggleRoman: () => void
}) {
  const { t } = useLingui()
  return (
    <div className={styles.keyRow}>
      ...
      <button
        type="button"
        className={styles.romanToggle}
        aria-pressed={roman === true}
        disabled={roman === undefined}
        title={
          roman === undefined
            ? t({
                id: 'chords.roman-needs-key',
                message: 'Nommer la tonalité de la grille pour lire en degrés'
              })
            : undefined
        }
        onClick={onToggleRoman}
      >
        {t({ id: 'chords.roman-toggle', message: 'Chiffrage romain' })}
      </button>
    </div>
  )
}
 ...
 * accounting stays true to the audible shift. Following rewrites the whole
 * grid in one click — like the detected draft, armed work deserves a
 * « Confirmer ? » beat before being rewritten.
 */
function PitchDriftFlag({
  source,
  pitchSemitones,
  transposedBy,
  onTranspose
  ...
  return (
    <>
      ...
      {/* The panel's one live region, mounted outside the conditional row:
...
export function ChordChartPanel({
  ...
  // Roman numerals (AN.5) — a reading preference like the layout: this
  // browser's habit, never chart data.
  const [roman, setRoman] = useState(() => readStoredRomanNumerals() ?? false)
  ...
  /** Flip the roman-numeral mode and remember the choice for next time. */
  function toggleRoman(): void {
    const next = !roman
    storeRomanNumerals(next)
    setRoman(next)
  }
  ...
  return (
    <section className={styles.panel}>
      ...
      <KeyRow
        ...
        onTranspose={onTranspose}
        onRespell={respell}
        roman={currentKey === undefined ? undefined : roman}
      ...
      <div className={styles.sheetViewport} data-sheet-scrollport>
        <LeadSheet
          ...
          barsPerRow={barsPerRow}
          romanKey={roman ? currentKey : undefined}
        />
      </div>
