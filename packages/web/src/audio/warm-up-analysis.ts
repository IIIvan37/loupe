 ...
 * No token (not signed in / not yet analysed this session) → skip; the first
 * real analysis then pays the cold start. Any failure is swallowed. Aborted
 * when the track is replaced (the caller's signal).
 */
export async function warmUpAnalysis(signal?: AbortSignal): Promise<void> {
  ...
  try {
    await fetch(`${analysisUrl()}/warmup`, {
      method: 'POST',
