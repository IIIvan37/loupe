 ...
 * (`/tempo`, `/chords`): encode the loaded PCM, upload it, fail loudly on a
 * non-2xx answer. Each adapter keeps only its response-shape guard and
 * mapping — a protocol change (auth, abort, error detail) lands once here.
 */
